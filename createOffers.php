...
if(count($params['pid']) > 0){
	...
	$offername = addslashes($_POST['offername']);
	} else {
		$offername = 'Offers XML Link';
	}
	//create offers id in db
	$nowdate = date('Y-m-d');
	$query = "INSERT INTO `".$prefix."mpxmloffers`(`offers_id`,`offer_name`, `date_added`) VALUES ('0','$offername','$nowdate')";
...
} else {echo 'Δεν επιλέξατε προϊόντα!';}
?>