...
<div class="alert alert-success clearfix">
    {l s='Your file has been successfully imported into your shop. Don\'t forget to re-build the products\' search index.' mod='mvxmldata'}
</div>
{/if}
<div class="row">
    <div class="col-lg-8">
        ...
        <div class="panel">
            ...
            <div class="alert alert-info">
                <ul class="list-unstyled">
                    <li>{l s='You can read information on import at:' mod='mvxmldata'}
                        <a href="{l s='http://doc.prestashop.com/display/PS17/Import' mod='mvxmldata'}" class="_blank">{l s='http://doc.prestashop.com/display/PS17/Import' mod='mvxmldata'}</a>
                    </li>
                    <li>{l s='Read more about the CSV format at:' mod='mvxmldata'}
                        <a href="{l s='https://en.wikipedia.org/wiki/Comma-separated_values' mod='mvxmldata'}" class="_blank">{l s='https://en.wikipedia.org/wiki/Comma-separated_values' mod='mvxmldata'}</a>
                    </li>
                </ul>
            </div>
            <hr />
            <form id="preview_import" action="{$current|escape:'html':'UTF-8'}&amp;token={$token|escape:'html':'UTF-8'}" method="post" enctype="multipart/form-data" class="form-horizontal">
                <div class="form-group">
                    <label for="entity" class="control-label col-lg-4">{l s='What do you want to import?' mod='mvxmldata'} </label>
                    <div class="col-lg-8">
                ...
                <div class="alert alert-warning import_products_categories">
                    <ul>
                        <li>{l s='Make Sure that there are NO conflicting rules. If there are any Conflicts, the module will consider the first Rule only' mod='mvxmldata'}</li>
                    </ul>
                </div>
                <div class="alert alert-warning import_supply_orders_details">
                    <p>{l s='Importing Supply Order Details will reset your history of ordered products, if there are any.' mod='mvxmldata'}</p>
                </div>
                <hr />
                <div class="form-group" id="csv_file_uploader">
                    <label for="file" class="control-label col-lg-4">{l s='Select a file to import' mod='mvxmldata'}</label>
                    <div class="col-lg-8">
                        ...
                        <button class="ladda-button btn btn-default" data-style="expand-right" data-size="s" type="button" id="file-add-button">
                            ...
                            {l s='Upload' mod='mvxmldata'}
                        </button>
                        {l s='or' mod='mvxmldata'}
                        <button class="btn btn-default csv-history-btn" type="button">
                            <span class="csv-history-nb badge">{$files_to_import|count}</span>
                            {l s='Choose from history / FTP' mod='mvxmldata'}
                        </button>
                        <p class="help-block">
                            {l s='Allowed formats: .csv, .xls, .xlsx, .xlst, .ods, .ots' mod='mvxmldata'}.<br/>
                            {l s='Only UTF-8 and ISO 8859-1 encodings are allowed' mod='mvxmldata'}.<br/>
                            {l s='You can also upload your file via FTP to the following directory: %s .' sprintf=[$path_import] mod='mvxmldata'}
                        </p>
                    </div>
                ...
                <div class="form-group" id="csv_files_history" style="display:none;" >
                    <div class="panel">
                        <div class="panel-heading">
                            {l s='History of uploaded files' mod='mvxmldata'}
                            <span class="csv-history-nb badge">{$files_to_import|count}</span>
                ...
                <div class="form-group">
                    <label for="iso_lang" class="control-label col-lg-4">
                        <span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='The locale must be installed' mod='mvxmldata'}">
                            {l s='Language of the file' mod='mvxmldata'}
                        </span>
                    </label>
                    <div class="col-lg-8">
                ...
                <div id="csv-fields">
                    <div class="form-group">
                        <label for="separator" class="control-label col-lg-4">{l s='Field separator' mod='mvxmldata'}</label>
                        <div class="col-lg-8">
                            ...
                            <div class="help-block">{l s='e.g. ' mod='mvxmldata'} 1; Blouse; 129.90; 5</div>
                        </div>
                        <div class="form-group">
                            <label for="multiple_value_separator" class="control-label col-lg-4">{l s='Multiple value separator' mod='mvxmldata'}</label>
                            <div class="col-lg-8">
                                ...
                                <div class="help-block">{l s='e.g. ' mod='mvxmldata'} Blouse; red.jpg, blue.jpg, green.jpg; 129.90</div>
                            </div>
                        </div>
                    </div>
                </div>
              ...
              '[/1]' => '</span>'
                    ...
                    <div class="col-lg-8">
                        <label class="switch-light prestashop-switch fixed-width-lg">
                            ...
                            <span>
                                <span>{l s='Yes' mod='mvxmldata'}</span>
                                <span>{l s='No' mod='mvxmldata'}</span>
                            </span>
                ...
                <div class="form-group" style="display: none">
                    <label for="match_ref" class="control-label col-lg-4">
                        <span data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='If enabled, the product\'s reference number MUST be unique!' mod='mvxmldata'}">
                            {l s='Use product reference as key' mod='mvxmldata'}
                        </span>
                    </label>
                    <div class="col-lg-8">
                        <label class="switch-light prestashop-switch fixed-width-lg">
                ...
                <div class="form-group">
                    <label for="regenerate" class="control-label col-lg-4">{l s='Skip thumbnails regeneration' mod='mvxmldata'}</label>
                    <div class="col-lg-8">
                        <label class="switch-light prestashop-switch fixed-width-lg">
                ...
                <div class="form-group">
                    <label for="forceIDs" class="control-label col-lg-4">
                        <span data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='If you enable this option, your imported items\' ID number will be used as-is. If you do not enable this option, the imported ID numbers will be ignored, and PrestaShop will instead create auto-incremented ID numbers for all the imported items.' mod='mvxmldata'}">
                            {l s='Force all ID numbers' mod='mvxmldata'}
                        </span>
                    </label>
                    <div class="col-lg-8">
                        <label class="switch-light prestashop-switch fixed-width-lg">
                ...
                <div class="form-group">
                    <label for="sendemail" class="control-label col-lg-4">
                        <span data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Sends an email to let you know your import is complete. It can be useful when handling large files, as the import may take some time.' mod='mvxmldata'}">
                            {l s='Send notification email' mod='mvxmldata'}</label>
                        </span>
                    <div class="col-lg-8">
                        <label class="switch-light prestashop-switch fixed-width-lg">
                ...
                <div class="panel-footer">
                    <button type="submit" name="submitImportFile" id="submitImportFile" class="btn btn-default pull-right" >
                        <i class="process-icon-next"></i> <span>{l s='Next step' mod='mvxmldata'}</span>
                    </button>
                </div>
            </form>
        </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="panel">
            <h3>
                ...
                {l s='Available fields' mod='mvxmldata'}
            </h3>
            <div id="availableFields" class="alert alert-info">
                {$available_fields}
            </div>
            <p>{l s='* Required field' mod='mvxmldata'}</p>
        </div>
        <div class="panel">
            <div class="panel-heading">
                <i class="icon-download"></i>
                {l s='Download sample csv files' mod='mvxmldata'}
            </div>

            <div class="list-group">
                <a class="list-group-item _blank" href="../docs/csv_import/categories_import.csv">
                    {l s='Sample Categories file' mod='mvxmldata'}
                </a>
            </div>
        </div>
    </div>
        ...
        $('#preview_import').submit(function(e) {
            if ($('#truncate').get(0).checked) {
                if (truncateAuthorized) {
                    if (!confirm('{l s='Are you sure that you would like to delete this entity: ' js=1 mod='mvxmldata'}' + ' ' + $.trim($('#entity > option:selected').text().toLowerCase()) + '?'))
                        e.preventDefault();
                }
                else {
                    jAlert('{l s='You do not have permission to delete this. When the MultiStore mode is enabled, only a SuperAdmin can delete all items before an import.' js=1 mod='mvxmldata'}');
                    return false;
                }
            }
        });
